...
def network_train(
    ...
    net_ts = network_init(
        no_LSTM_layers, no_LSTM_units, batch_size, timesteps, no_feat, loss, opt
    )
    ...
    while i <= no_epochs:
        ...
        if i == min_epochs:
            ...
            # save model weights
            net_ts.save_weights(
                filepath="model_weights_temp/best_model_weights.hdf5", overwrite=True
            )
        ...
        if i > min_epochs:
            if current_valid_loss < best_valid_loss:
                ...
                # save model weights
                net_ts.save_weights(
                    filepath="model_weights_temp/best_model_weights.hdf5",
                    overwrite=True,
                )
    ...
    net.load_weights("./model_weights_temp/best_model_weights.hdf5")
    net.save(filepath="final_model.h5", overwrite=True)
