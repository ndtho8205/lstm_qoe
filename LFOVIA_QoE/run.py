...
for test_video_no in range(1, no_videos + 1):
    ...
    for ii in range(no_training_videos):
        ...
        video_content = sio.loadmat(path1 + "video_%d.mat" % (training_video_no))

        STSQ = np.transpose(video_content["STRRED_ext"])  # FIXME: NIQE_ext
        Nrebuffers = np.transpose(video_content["Nrebuffers"])
        TSL = np.transpose(video_content["TSL"])
        ...
        score_continuous = np.transpose(video_content["score_continuous"])
        score_continuous_CIhigh = np.transpose(video_content["score_continuous_CIhigh"])
        score_continuous_CIlow = np.transpose(video_content["score_continuous_CIlow"])

        score_continuous_norm = (score_continuous - score_continuous_min) / (
            score_continuous_max - score_continuous_min
        )
        score_continuous_CIhigh_norm = (
            score_continuous_CIhigh - score_continuous_min
        ) / (score_continuous_max - score_continuous_min)
        score_continuous_CIlow_norm = (
            score_continuous_CIlow - score_continuous_min
        ) / (score_continuous_max - score_continuous_min)

        video_features = STSQ_norm
        video_features = np.hstack((video_features, Nrebuffers))
    ...
    training_features2 = training_features[
        0 : len(training_features) // timesteps * timesteps, :
    ]
    training_labels2 = training_labels[
        0 : len(training_labels) // timesteps * timesteps, :
    ]

    training_features_ts = np.reshape(
        training_features2, (-1, timesteps, training_features2.shape[1])
    )
    training_labels_ts = np.reshape(
        training_labels2, (-1, timesteps, training_labels2.shape[1])
    )
    ...
    for ii in range(no_validation_videos):
        ...
        video_content = sio.loadmat(path1 + "video_%d.mat" % (validation_video_no))

        STSQ = np.transpose(video_content["STRRED_ext"])
        Nrebuffers = np.transpose(video_content["Nrebuffers"])
        TSL = np.transpose(video_content["TSL"])
        ...
        validation_features2 = validation_features[
            0 : len(validation_features) // timesteps * timesteps, :
        ]
        validation_labels2 = validation_labels[
            0 : len(validation_labels) // timesteps * timesteps, :
        ]

        validation_features_ts = np.reshape(
            validation_features2, (-1, timesteps, validation_features2.shape[1])
        )
        validation_labels_ts = np.reshape(
            validation_labels2, (-1, timesteps, validation_labels2.shape[1])
        )

    ############# Testing ######################

    video_content = sio.loadmat(path1 + "video_%d.mat" % (test_video_no))
    ...
    score_continuous_CIhigh_norm = (score_continuous_CIhigh - score_continuous_min) / (
        score_continuous_max - score_continuous_min
    )
    score_continuous_CIlow_norm = (score_continuous_CIlow - score_continuous_min) / (
        score_continuous_max - score_continuous_min
    )

    epsilon_test = np.asarray(
        score_continuous_CIhigh_norm - score_continuous_CIlow_norm
    )
    ...
    net = network_train(
        ...
        loss,
        opt,
        validation_features_ts,
        validation_labels_ts,
    )
    ...
    np.savetxt(
        f"testX_{1}_{test_features.shape[0]}_{test_features.shape[1]}_{test_features.shape[2]}.txt",
        test_features.flatten(),
        delimiter="\n",
    )
    np.savetxt(f"testY_{1}.txt", actual_QoE.flatten(), delimiter="\n")
    ...
    pred_QoE = pred_QoE.reshape((-1,))  # TODO: added
    actual_QoE = actual_QoE.reshape((-1,))  # TODO: added
